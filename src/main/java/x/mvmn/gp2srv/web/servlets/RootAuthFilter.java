...
public final class RootAuthFilter implements Filter {

	private final String username;
	private final String password;

	public RootAuthFilter(final String username, final String password) {
		...
		if (request instanceof HttpServletRequest && response instanceof HttpServletResponse) {
			...
			if (!httpRequest.getRequestURI().matches("/")) {
				...
				if (loggedIn != null && loggedIn.equals(Boolean.TRUE)) {
					...
					httpReponse.setHeader("WWW-Authenticate", "Basic realm=\"Admin user\"");
				}
			}
		} else {
			chain.doFilter(request, response);
