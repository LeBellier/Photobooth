 ...
 *
 * The camera sends a continuous stream of UDP packets to whoever called its
 * "startstream" method
 */
public class StreamViewer implements Runnable {
     ...
     *
     * @param imageConsumer        the consumer to receive the BufferedImages
	...
	// The image data starts somewhere after the first 130 bytes, but at last in 320
	// bytes
	for (int k = 130; k < 320 && k < (receivedPacket.getLength() - 1); k++) {
	    // The bytes FF and D8 signify the start of the jpeg data, see
    ...
    @Override
    public void run() {
	// The camera sends each image in one UDP packet, normally between 25000 and
	// 30000 bytes. We set 35000 here to be safe.
