...
public class NetUtil {
    ...
    /**
     * Find a local interface IPv4 address that lies in the same subnet as the given
     * IP address.
     *
	...
	throw new SocketException(
		"Could not find any local IP address in subnet " + anyIpInSubnet + "/" + subnetMaskBitSize + " .");
    }
     ...
     * @param ip2            The second IPv4 address (converted by
     *                       #inetAddrToInt()).
    ...
    /**
     * Convert the given InetAddr to the integer corresponding to the IPv4 address
     * (if it is an IPv4 address).
     *
     * @param address The address to convert.
     * @return The integer corresponding to the given address, or zero if the
     *         address is not IPv4.
     */
    private static int inetAddrToInt(InetAddress address) {
	if (address instanceof Inet4Address) {
	    byte[] ipv4Address = ((Inet4Address) address).getAddress();
	    return ((ipv4Address[0] & 0xFF) << 24) | ((ipv4Address[1] & 0xFF) << 16) | ((ipv4Address[2] & 0xFF) << 8)
		    | ((ipv4Address[3] & 0xFF));
	} else {
	    return 0;
	}
