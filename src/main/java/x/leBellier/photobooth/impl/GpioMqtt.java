...
public class GpioMqtt implements GpioService, MqttCallback {
	...
	try {
	    ...
	    MqttConnectOptions mqOptions = new MqttCredentials();
	...
	try {
	    System.out.println(String.format("Send Cmd : %s", cmd));
...
//				logger.trace(response.toString());
	    ...
	    System.out.println("Aucun chemin d'accès pour atteindre l'hôte cible (Host unreachable)");
	} catch (IOException e) {
	    e.printStackTrace();
	}
    }
	...
	System.out.println(
		String.format("Mqtt receive a message : Topic = %s / Message = %s", topic, message.toString()));
